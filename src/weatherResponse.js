...
export const hourWeather = async (param) => {
    const url = `https://api.weatherbit.io/v2.0/forecast/hourly?${param}&key=${apiKey}&hours=5`;
    const fetching = await fetch(url);
...
export const daysWeather = async (param) => {
    const url = `https://api.weatherbit.io/v2.0/forecast/daily?${param}&key=${apiKey}&days=5`;
    const fetching = await fetch(url);
